...
subtitle: "GMBA Case Study Report (2000)"
date: "June 2026"
format:
  pdf:
...
**Debate Proposition**: "Was Netflix's threat predictable, and what should Blockbuster have done differently?"
...
Blockbuster failed to acquire Netflix and continued defending a store-based rental model while streaming and subscription economics reshaped home entertainment. The surrounding industry environment made the decision more difficult because competitors, customers, investors, and regulators were already changing expectations about speed, transparency, and value creation.

## Company's Strategic Position

Blockbuster entered the case with valuable assets: brand recognition, customer relationships, distribution, data, and managerial attention. Those assets also created inertia. The same strengths that made the company important made it harder to admit that an existing strategy, product, policy, or governance model might need to change.
...
The core business problem is how Blockbuster should respond when evidence suggests that the current approach is producing strategic, ethical, or operational risk. The case connects directly to Disruption, strategic myopia, business model innovation, but the management dilemma is broader: leaders must decide whether the problem is a temporary execution failure or a sign that the underlying model needs redesign.
...
\toprule
\rowcolor{lightgray}
\textbf{Technique} & \textbf{Application at Blockbuster} \\
\midrule
\endhead
...
- Disruption, strategic myopia, business model innovation provides a theoretical basis for analyzing the decision.
...
- Netflix, Inc. (n.d.). *Annual reports and proxies*. <https://ir.netflix.net/financials/annual-reports-and-proxies/default.aspx>
  - Relevance: Provides official Netflix financial history for comparing the rejected acquisition with later scale.
